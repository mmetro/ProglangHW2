...
declare
fun {SimulatePlayer Deck DiscardPile Hand Bet}
   local Move={PlayerMove Hand} in 
      if Move==hit then
	 case {DealCard Deck DiscardPile} of C|D|DP then
	    {SimulatePlayer D DP {List.append Hand [C]} Bet}
	 end
...
declare
fun {SimulatePlayers Deck DiscardPile FinishedHands UnfinishedHands FinalBets OriginalBets B}
   case UnfinishedHands of H|T then
      case OriginalBets of H1|T1 then
	 case {SimulatePlayer Deck DiscardPile H H1} of D|DP|Hand|B then
	    {SimulatePlayers D DP {List.append FinishedHands [Hand]} T {List.append FinalBets [B]} T1 B}
	 end
      end	 
   else
      Deck|DiscardPile|FinishedHands|FinalBets
   end
end

declare
fun {SimulateDealer Deck DiscardPile Hand}
   if {ValueOfHand Hand}<17 then
      case {DealCard Deck DiscardPile} of C|D|DP then
...
fun {BetListRecurse RemainingPlayers BetAmount Bets}
 if RemainingPlayers==1 then
      ...
      {BetListRecurse {List.append Hands [[Bets]]} RemainingPlayers-1}
   end
end
...
%{Browse {ShuffleFromIndex 1 {List.number 1 52 1} 52}}   
